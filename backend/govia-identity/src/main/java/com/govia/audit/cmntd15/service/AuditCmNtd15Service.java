package com.govia.audit.cmntd15.service;
...
/** CRUD + Import/Export cho "Danh sách các bút toán chọn mẫu nghiệp vụ MF" (sheet ZTC_CM_NTD15, phu
 * luc 05D/BKS-KTNB) - cung cau truc voi AuditCmNtd12Service, xem AuditCmNtd15. */
@Service
public class AuditCmNtd15Service {
    ...
    private final AuditCmNtd15Repository repository;
    ...
    public AuditCmNtd15Service(AuditCmNtd15Repository repository, AuditEngagementRepository engagementRepository,
    ...
    public List<AuditCmNtd15Response> list(UUID engagementId) {
    ...
    @Transactional
    public AuditCmNtd15Response create(AuditCmNtd15Request request) {
        UUID tenantId = TenantContext.getTenantId();
        ...
        AuditCmNtd15 item = new AuditCmNtd15();
        item.setTenantId(tenantId);
        applyRequest(item, request);
        item = repository.save(item);

        auditLogService.record("AuditCmNtd15", item.getId(), AuditAction.CREATE,
                "Tao ban ghi but toan chon mau MF: " + item.getBranchCode() + " - " + item.getEntryNumber());
        return toResponse(item, buildResponseContext(tenantId));
    }

    @Transactional
    public AuditCmNtd15Response update(UUID id, AuditCmNtd15Request request) {
        UUID tenantId = TenantContext.getTenantId();
        AuditCmNtd15 item = getOwnedOrThrow(tenantId, id);
        ...
        auditLogService.record("AuditCmNtd15", item.getId(), AuditAction.UPDATE,
                "Cap nhat ban ghi but toan chon mau MF: " + item.getBranchCode() + " - " + item.getEntryNumber());
        return toResponse(item, buildResponseContext(tenantId));
    }
    ...
    public void delete(UUID id) {
        ...
        auditLogService.record("AuditCmNtd15", id, AuditAction.DELETE,
                "Xoa ban ghi but toan chon mau MF: " + item.getBranchCode() + " - " + item.getEntryNumber());
    }

    @Transactional(readOnly = true)
    public byte[] exportExcel(UUID engagementId) {
        return excelExportService.export("audit_cm_ntd15", exportColumns(), exportRows(engagementId));
    }

    @Transactional(readOnly = true)
    public byte[] exportWord(UUID engagementId) {
        return wordExportService.export("Danh sách các bút toán chọn mẫu nghiệp vụ MF (05D)", exportColumns(), exportRows(engagementId));
    }
    ...
    public ImportResult importFromExcel(UUID engagementId, MultipartFile file) {
        ...
        for (int i = 0; i < rows.size(); i++) {
            ...
            try {
                ...
                Optional<AuditCmNtd15> existing = repository.findByTenantIdAndBranchCodeAndTransactionDateAndPostingUserAndEntryNumber(
                        tenantId, branchCode.trim(), transactionDate, postingUser.trim(), entryNumber);
                AuditCmNtd15Request request = new AuditCmNtd15Request(engagementId,
                        ...
                        emptyToNull(row.get("controlStaffTitle")), existing.map(AuditCmNtd15::isActive).orElse(true));
        ...
        auditLogService.record("AuditCmNtd15", null, AuditAction.CREATE,
                "Import Excel but toan chon mau MF: " + success + " thanh cong, " + errors.size() + " loi");
        return new ImportResult(success, errors.size(), errors);
    }

    private void applyRequest(AuditCmNtd15 item, AuditCmNtd15Request request) {
    ...
    private void checkNoDuplicate(UUID tenantId, String branchCode, LocalDate transactionDate, String postingUser, BigDecimal entryNumber, UUID excludingId) {
        repository.findByTenantIdAndBranchCodeAndTransactionDateAndPostingUserAndEntryNumber(tenantId, branchCode, transactionDate, postingUser, entryNumber)
                ...
                .ifPresent(existing -> {
                    throw new BusinessException("AUDIT_CM_NTD15_DUPLICATE", "Ban ghi nay da ton tai: " + entryNumber);
                });
    }

    private AuditCmNtd15 getOwnedOrThrow(UUID tenantId, UUID id) {
        return repository.findById(id)
                .filter(item -> item.getTenantId().equals(tenantId))
                .orElseThrow(() -> new BusinessException("AUDIT_CM_NTD15_NOT_FOUND", "Khong tim thay ban ghi but toan chon mau", HttpStatus.NOT_FOUND));
    }
    ...
    private AuditCmNtd15Response toResponse(AuditCmNtd15 item, ResponseContext ctx) {
        Employee assignee = item.getAssignedEmployeeId() == null ? null : ctx.employees().get(item.getAssignedEmployeeId());
        AuditProcessStepSummary step = item.getProcessStepSummaryId() == null ? null : ctx.stepSummaries().get(item.getProcessStepSummaryId());
        return new AuditCmNtd15Response(item.getId(),
