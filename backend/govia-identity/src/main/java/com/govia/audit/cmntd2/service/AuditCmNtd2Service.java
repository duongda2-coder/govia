...
public class AuditCmNtd2Service {
    ...
    public AuditCmNtd2Service(AuditCmNtd2Repository repository, AuditLogService auditLogService,
    ...
    public List<AuditCmNtd2Response> list() {
    ...
    public byte[] exportExcel() {
        return excelExportService.export("audit_cm_ntd2", exportColumns(), exportRows());
    }

    @Transactional(readOnly = true)
    public byte[] exportWord() {
        return wordExportService.export("Danh sách chọn mẫu giao dịch về nghiệp vụ HĐV", exportColumns(), exportRows());
    }
    ...
    public ImportResult importFromExcel(MultipartFile file) {
        ...
        for (int i = 0; i < rows.size(); i++) {
            ...
            try {
                ...
                AuditCmNtd2Request request = new AuditCmNtd2Request(branchCode.trim(), transactionDate, parseDate(row.get("valueDate")),
    ...
    private AuditCmNtd2Response toResponse(AuditCmNtd2 item) {
        return new AuditCmNtd2Response(item.getId(), item.getBranchCode(), item.getTransactionDate(), item.getValueDate(),
