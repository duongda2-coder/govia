...
public class AuditCmNtd12Service {
    ...
    public AuditCmNtd12Service(AuditCmNtd12Repository repository, AuditEngagementRepository engagementRepository,
    ...
    public List<AuditCmNtd12Response> list(UUID engagementId) {
    ...
    public byte[] exportExcel(UUID engagementId) {
        return excelExportService.export("audit_cm_ntd12", exportColumns(), exportRows(engagementId));
    }

    @Transactional(readOnly = true)
    public byte[] exportWord(UUID engagementId) {
        return wordExportService.export("Danh các bút toán chọn mẫu TCKT", exportColumns(), exportRows(engagementId));
    }
    ...
    public ImportResult importFromExcel(UUID engagementId, MultipartFile file) {
        ...
        for (int i = 0; i < rows.size(); i++) {
            ...
            try {
                ...
                AuditCmNtd12Request request = new AuditCmNtd12Request(engagementId,
    ...
    private AuditCmNtd12Response toResponse(AuditCmNtd12 item, ResponseContext ctx) {
        Employee assignee = item.getAssignedEmployeeId() == null ? null : ctx.employees().get(item.getAssignedEmployeeId());
        AuditProcessStepSummary step = item.getProcessStepSummaryId() == null ? null : ctx.stepSummaries().get(item.getProcessStepSummaryId());
        return new AuditCmNtd12Response(item.getId(),
