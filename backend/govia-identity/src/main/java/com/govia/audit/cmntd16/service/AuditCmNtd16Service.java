package com.govia.audit.cmntd16.service;
...
/** CRUD + Import/Export cho "Danh sách các bút toán chọn mẫu nghiệp vụ HDV" (sheet ZTC_CM_NTD16, phu
 * luc 03G/BKS-KTNB) - cung cau truc voi AuditCmNtd12Service/AuditCmNtd15Service, xem AuditCmNtd16. */
@Service
public class AuditCmNtd16Service {
    ...
    private final AuditCmNtd16Repository repository;
    ...
    public AuditCmNtd16Service(AuditCmNtd16Repository repository, AuditEngagementRepository engagementRepository,
    ...
    public List<AuditCmNtd16Response> list(UUID engagementId) {
    ...
    @Transactional
    public AuditCmNtd16Response create(AuditCmNtd16Request request) {
        UUID tenantId = TenantContext.getTenantId();
        ...
        AuditCmNtd16 item = new AuditCmNtd16();
        item.setTenantId(tenantId);
        applyRequest(item, request);
        item = repository.save(item);

        auditLogService.record("AuditCmNtd16", item.getId(), AuditAction.CREATE,
                "Tao ban ghi but toan chon mau HDV: " + item.getBranchCode() + " - " + item.getEntryNumber());
        return toResponse(item, buildResponseContext(tenantId));
    }

    @Transactional
    public AuditCmNtd16Response update(UUID id, AuditCmNtd16Request request) {
        UUID tenantId = TenantContext.getTenantId();
        AuditCmNtd16 item = getOwnedOrThrow(tenantId, id);
        ...
        auditLogService.record("AuditCmNtd16", item.getId(), AuditAction.UPDATE,
                "Cap nhat ban ghi but toan chon mau HDV: " + item.getBranchCode() + " - " + item.getEntryNumber());
        return toResponse(item, buildResponseContext(tenantId));
    }
    ...
    public void delete(UUID id) {
        ...
        auditLogService.record("AuditCmNtd16", id, AuditAction.DELETE,
                "Xoa ban ghi but toan chon mau HDV: " + item.getBranchCode() + " - " + item.getEntryNumber());
    }

    @Transactional(readOnly = true)
    public byte[] exportExcel(UUID engagementId) {
        return excelExportService.export("audit_cm_ntd16", exportColumns(), exportRows(engagementId));
    }

    @Transactional(readOnly = true)
    public byte[] exportWord(UUID engagementId) {
        return wordExportService.export("Danh sách các bút toán chọn mẫu nghiệp vụ HDV (03G)", exportColumns(), exportRows(engagementId));
    }
    ...
    public ImportResult importFromExcel(UUID engagementId, MultipartFile file) {
        ...
        for (int i = 0; i < rows.size(); i++) {
            ...
            try {
                ...
                Optional<AuditCmNtd16> existing = repository.findByTenantIdAndBranchCodeAndTransactionDateAndPostingUserAndEntryNumber(
                        tenantId, branchCode.trim(), transactionDate, postingUser.trim(), entryNumber);
                AuditCmNtd16Request request = new AuditCmNtd16Request(engagementId,
                        ...
                        branchCode.trim(), transactionDate, postingUser.trim(), entryNumber,
                        ...
                        emptyToNull(row.get("controlStaffTitle")), existing.map(AuditCmNtd16::isActive).orElse(true));
        ...
        auditLogService.record("AuditCmNtd16", null, AuditAction.CREATE,
                "Import Excel but toan chon mau HDV: " + success + " thanh cong, " + errors.size() + " loi");
        return new ImportResult(success, errors.size(), errors);
    }

    private void applyRequest(AuditCmNtd16 item, AuditCmNtd16Request request) {
    ...
    private void checkNoDuplicate(UUID tenantId, String branchCode, LocalDate transactionDate, String postingUser, BigDecimal entryNumber, UUID excludingId) {
        repository.findByTenantIdAndBranchCodeAndTransactionDateAndPostingUserAndEntryNumber(tenantId, branchCode, transactionDate, postingUser, entryNumber)
                ...
                .ifPresent(existing -> {
                    throw new BusinessException("AUDIT_CM_NTD16_DUPLICATE", "Ban ghi nay da ton tai: " + entryNumber);
                });
    }

    private AuditCmNtd16 getOwnedOrThrow(UUID tenantId, UUID id) {
        return repository.findById(id)
                .filter(item -> item.getTenantId().equals(tenantId))
                .orElseThrow(() -> new BusinessException("AUDIT_CM_NTD16_NOT_FOUND", "Khong tim thay ban ghi but toan chon mau", HttpStatus.NOT_FOUND));
    }
    ...
    private List<ExportColumn> exportColumns() {
        return List.of(
                ...
                new ExportColumn("transactionStatus", "Mã trạng thái"),
    ...
    private AuditCmNtd16Response toResponse(AuditCmNtd16 item, ResponseContext ctx) {
        Employee assignee = item.getAssignedEmployeeId() == null ? null : ctx.employees().get(item.getAssignedEmployeeId());
        AuditProcessStepSummary step = item.getProcessStepSummaryId() == null ? null : ctx.stepSummaries().get(item.getProcessStepSummaryId());
        return new AuditCmNtd16Response(item.getId(),
