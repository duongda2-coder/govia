...
export function CmNtd13Page() {
  ...
  const load = useCallback(
    async (selectedEngagementId: string) => {
      setLoading(true);
      try {
        setItems(await listAuditCmNtd13(selectedEngagementId));
      } catch {
  ...
  const columns: TableProps<AuditCmNtd13Item>["columns"] = [
    { title: t("auditCmNtd13.columns.assignedUsername"), width: 150, ...getSearchColumnProps("assignedUsername", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd13.columns.processStepSummaryCode"), width: 130, ...getSearchColumnProps("processStepSummaryCode", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd13.columns.branchCode"), width: 110, ...getSearchColumnProps("branchCode", searchLabels) },
    { title: t("auditCmNtd13.columns.occurrenceDate"), width: 120, ...getSearchColumnProps("occurrenceDate", searchLabels) },
    { title: t("auditCmNtd13.columns.merchantId"), width: 140, ...getSearchColumnProps("merchantId", searchLabels), render: (v: string | null) => v ?? "-" },
    ...
    { title: t("auditCmNtd13.columns.status"), width: 120, ...getSearchColumnProps("status", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd13.columns.sampleReason"), width: 130, ...getSearchColumnProps("sampleReason", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd13.columns.auditResult"), ...getSearchColumnProps("auditResult", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd13.columns.recommendationType"), ...getSearchColumnProps("recommendationType", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd13.columns.transactionStaff"), width: 140, ...getSearchColumnProps("transactionStaff", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd13.columns.controlUser"), width: 140, ...getSearchColumnProps("controlUser", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd13.columns.controlStaff"), width: 140, ...getSearchColumnProps("controlStaff", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd13.columns.controlStaffTitle"), width: 160, ...getSearchColumnProps("controlStaffTitle", searchLabels), render: (v: string | null) => v ?? "-" },
    {
  ...
  return (
    <div>
      ...
      <CrudTable<AuditCmNtd13Item>
        ...
        onExportExcel={canExport && engagementId ? () => exportAuditCmNtd13("excel", engagementId) : undefined}
        onExportWord={canExport && engagementId ? () => exportAuditCmNtd13("word", engagementId) : undefined}
        onImport={
          canImport && engagementId
            ? async (file) => {
                const result = await importAuditCmNtd13(engagementId, file);
                await load(engagementId);
                return result;
              }
            : undefined
      ...
      >
        <Form<FormValues> form={form} layout="vertical">
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="assignedEmployeeId" label={t("auditCmNtd13.columns.assignedUsername")}>
                <Select
                  allowClear
                  showSearch
                  optionFilterProp="label"
            ...
            <Col span={12}>
              <Form.Item name="processStepSummaryId" label={t("auditCmNtd13.columns.processStepSummaryCode")}>
                <Select
                  allowClear
                  showSearch
                  optionFilterProp="label"
