...
export function CmNtd8Page() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setItems(await listAuditCmNtd8());
    } catch {
  ...
  return (
    <div>
      ...
      <CrudTable<AuditCmNtd8Item>
        ...
        onExportExcel={canExport ? () => exportAuditCmNtd8("excel") : undefined}
        onExportWord={canExport ? () => exportAuditCmNtd8("word") : undefined}
        onImport={
          canImport
            ? async (file) => {
                const result = await importAuditCmNtd8(file);
                await load();
                return result;
              }
            : undefined
