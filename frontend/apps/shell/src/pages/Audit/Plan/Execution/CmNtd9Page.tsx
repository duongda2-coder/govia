...
export function CmNtd9Page() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setItems(await listAuditCmNtd9());
    } catch {
  ...
  return (
    <div>
      ...
      <CrudTable<AuditCmNtd9Item>
        ...
        onExportExcel={canExport ? () => exportAuditCmNtd9("excel") : undefined}
        onExportWord={canExport ? () => exportAuditCmNtd9("word") : undefined}
        onImport={
          canImport
            ? async (file) => {
                const result = await importAuditCmNtd9(file);
                await load();
                return result;
              }
            : undefined
