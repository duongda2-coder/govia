...
export function CmNtd14Page() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      setItems(await listAuditCmNtd14());
    } catch {
  ...
  return (
    <div>
      ...
      <CrudTable<AuditCmNtd14Item>
        ...
        onExportExcel={canExport ? () => exportAuditCmNtd14("excel") : undefined}
        onExportWord={canExport ? () => exportAuditCmNtd14("word") : undefined}
        onImport={
          canImport
            ? async (file) => {
                const result = await importAuditCmNtd14(file);
                await load();
                return result;
              }
            : undefined
