...
export function CmNtd1Page() {
  ...
  const load = useCallback(
    async (selectedEngagementId: string) => {
      setLoading(true);
      try {
        setItems(await listAuditCmNtd1(selectedEngagementId));
      } catch {
  ...
  const columns: TableProps<AuditCmNtd1Item>["columns"] = [
    { title: t("auditCmNtd1.columns.assignedUsername"), width: 150, ...getSearchColumnProps("assignedUsername", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd1.columns.processStepSummaryCode"), width: 130, ...getSearchColumnProps("processStepSummaryCode", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd1.columns.branchCode"), width: 110, ...getSearchColumnProps("branchCode", searchLabels) },
    { title: t("auditCmNtd1.columns.transactionDate"), width: 130, ...getSearchColumnProps("transactionDate", searchLabels) },
    { title: t("auditCmNtd1.columns.postingUser"), width: 120, ...getSearchColumnProps("postingUser", searchLabels) },
    { title: t("auditCmNtd1.columns.entryNumber"), dataIndex: "entryNumber", width: 120, align: "right", sorter: (a, b) => num(a.entryNumber, b.entryNumber), render: money },
    { title: t("auditCmNtd1.columns.debitAmount"), dataIndex: "debitAmount", width: 150, align: "right", sorter: (a, b) => num(a.debitAmount, b.debitAmount), render: money },
    { title: t("auditCmNtd1.columns.creditAmount"), dataIndex: "creditAmount", width: 150, align: "right", sorter: (a, b) => num(a.creditAmount, b.creditAmount), render: money },
    { title: t("auditCmNtd1.columns.transactionStatus"), width: 140, ...getSearchColumnProps("transactionStatus", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd1.columns.currency"), width: 90, ...getSearchColumnProps("currency", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd1.columns.accountNumber"), width: 130, ...getSearchColumnProps("accountNumber", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd1.columns.content"), ...getSearchColumnProps("content", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd1.columns.sampleReason"), width: 130, ...getSearchColumnProps("sampleReason", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd1.columns.auditResult"), ...getSearchColumnProps("auditResult", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd1.columns.recommendationType"), width: 150, ...getSearchColumnProps("recommendationType", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd1.columns.transactionStaff"), width: 150, ...getSearchColumnProps("transactionStaff", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd1.columns.controlUser"), width: 130, ...getSearchColumnProps("controlUser", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd1.columns.controlStaff"), width: 150, ...getSearchColumnProps("controlStaff", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd1.columns.controlStaffTitle"), width: 170, ...getSearchColumnProps("controlStaffTitle", searchLabels), render: (v: string | null) => v ?? "-" },
    { title: t("auditCmNtd1.columns.workType"), width: 130, ...getSearchColumnProps("workType", searchLabels), render: (v: string | null) => v ?? "-" },
    {
  ...
  return (
    <div>
      ...
      <CrudTable<AuditCmNtd1Item>
        ...
        onExportExcel={canExport && engagementId ? () => exportAuditCmNtd1("excel", engagementId) : undefined}
        onExportWord={canExport && engagementId ? () => exportAuditCmNtd1("word", engagementId) : undefined}
        onImport={
          canImport && engagementId
            ? async (file) => {
                const result = await importAuditCmNtd1(engagementId, file);
                await load(engagementId);
                return result;
              }
            : undefined
      ...
      >
        <Form<FormValues> form={form} layout="vertical">
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="assignedEmployeeId" label={t("auditCmNtd1.columns.assignedUsername")}>
                <Select
                  allowClear
                  showSearch
                  optionFilterProp="label"
            ...
            <Col span={12}>
              <Form.Item name="processStepSummaryId" label={t("auditCmNtd1.columns.processStepSummaryCode")}>
                <Select
                  allowClear
                  showSearch
                  optionFilterProp="label"
