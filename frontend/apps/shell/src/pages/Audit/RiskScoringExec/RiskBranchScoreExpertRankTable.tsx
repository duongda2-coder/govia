...
import {
  riskBranchScoreExpertRankApi,
  type RiskBranchScoreExpertRankItem,
  type RiskBranchScoreExpertRankRequest,
...
/**
 * "Xep hang rui ro chi nhanh theo y kien chuyen gia" (sheet ZTC_DGRR_cg) - 2 thao tac theo dung
 * tai lieu goc: "Cap nhat du lieu tu nguon" (keo lai Tong diem/Xep hang tu Ket qua cham diem tong
 * hop, khong dung vao du lieu chuyen gia da nhap) va "Xem ket qua" (tai lai danh sach da luu). Sua
 * tung dong (Xep hang lai theo YKCG, Ly do, Ngay danh gia, Chuyen gia, Ket qua xep loai) qua modal -
 * khong co Them/Xoa vi vong doi dong do "Cap nhat du lieu tu nguon" quan ly.
 */
export function RiskBranchScoreExpertRankTable() {
  ...
  const { getSearchColumnProps } = useClientSearchColumn<RiskBranchScoreExpertRankItem>();
  const searchLabels = { confirmText: t("common.search"), resetText: t("common.reset") };

  const [years, setYears] = useState<MasterDataItem[]>([]);
  const [year, setYear] = useState<number | undefined>(undefined);
  const [rankLabels, setRankLabels] = useState<string[]>([]);
  const [items, setItems] = useState<RiskBranchScoreExpertRankItem[]>([]);
  const [loading, setLoading] = useState(false);
  const [syncing, setSyncing] = useState(false);
  const [selected, setSelected] = useState<RiskBranchScoreExpertRankItem[]>([]);
  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState<RiskBranchScoreExpertRankItem | null>(null);
  ...
  const load = useCallback(
    async (selectedYear: number) => {
      setLoading(true);
      try {
        setItems(await riskBranchScoreExpertRankApi.list(selectedYear));
      } catch {
  ...
  const handleSync = async () => {
    ...
    setSyncing(true);
    try {
      const list = await riskBranchScoreExpertRankApi.sync(year);
      setItems(list);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const request: RiskBranchScoreExpertRankRequest = {
      ...
      const updated = await riskBranchScoreExpertRankApi.update(editing.id, request);
  ...
  const columns: CrudColumn<RiskBranchScoreExpertRankItem>[] = [
  ...
  return (
    <div>
      ...
      <CrudTable<RiskBranchScoreExpertRankItem>
        tableId="riskScoringExec.branchScoreExpertRank"
        columns={columns}
        dataSource={items}
        rowKey="id"
        loading={loading}
